...
export const VendorModule: React.FC = () => {
        ...
        return (
            <div className="h-full flex items-center justify-center bg-slate-50 p-4 animate-in fade-in">
                <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 p-10 text-center scale-100 animate-in zoom-in-95">
                    ...
                    <h2 className="text-2xl font-playfair font-bold tracking-tight text-slate-800 mb-3 uppercase tracking-tight">Supplier Vault Lock</h2>
 ...
 <input type="password" placeholder="SECURITY KEY" className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-[2rem] outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 font-bold text-center tracking-[0.5em] transition-all" value={password} onChange={(e) => setPassword(e.target.value)} autoFocus />
                        <button type="submit" className="w-full bg-indigo-600 text-white font-black py-5 rounded-[2rem] shadow-xl shadow-indigo-500/20 uppercase tracking-[0.2em] text-xs hover:bg-indigo-700 transition-all active:scale-95">Decrypt & Access</button>
                    </form>
                </div>
            </div>
        );
    ...
    return (
        <div className="h-full flex flex-col gap-4 overflow-hidden p-2">
            <div className="bg-slate-100 p-1.5 rounded-[2.5rem] border border-slate-200 shadow-inner w-fit shrink-0 flex gap-1">
                <button onClick={() => { setViewState('stock'); setActiveFormTab('profile'); }} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-[2rem] transition-all flex items-center gap-2 ${viewState === 'stock' ? 'bg-emerald-900 text-white shadow-[0_10px_20px_-5px_rgba(6,78,59,0.5)] scale-100' : 'text-slate-400 hover:text-emerald-700 scale-95'}`}><List size={14} /> Registry</button>
                <button onClick={() => { setEditingId(null); setViewState('builder'); setBuilderMode('add'); setVendor(DEFAULT_VENDOR); setActiveFormTab('profile'); }} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-[2rem] transition-all flex items-center gap-2 ${viewState === 'builder' && builderMode === 'add' ? 'bg-emerald-900 text-white shadow-[0_10px_20px_-5px_rgba(6,78,59,0.5)] scale-100' : 'text-slate-400 hover:text-emerald-700 scale-95'}`}><Plus size={14} /> Intake</button>
            </div>
                ...
                <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-300 shadow-sm overflow-hidden flex flex-col animate-in fade-in">
                    <div className="p-5 border-b border-slate-300 bg-slate-50/30 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-indigo-100 rounded-[2rem] flex items-center justify-center text-indigo-600 shadow-inner border border-indigo-50"><Truck size={20} /></div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredVendors.map((v, idx) => (
                                    <tr key={v.id} className="hover:bg-slate-50 transition-colors group cursor-pointer" onClick={() => { setVendor(v); setEditingId(v.id); setViewState('builder'); setBuilderMode('edit'); setActiveFormTab('profile'); }}>
                                        ...
                                        <td className="px-8 py-6"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-[2rem] bg-slate-100 border border-slate-200 flex items-center justify-center font-black text-slate-400 uppercase text-[14px]">{v.name.charAt(0)}</div><div><div className="font-black text-slate-800 uppercase text-[13px] tracking-tight">{v.name}</div><div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{v.id}</div></div></div></td>
 ...
 <div className="flex flex-col"><h3 className="text-2xl font-bold tracking-tight text-slate-800 uppercase tracking-tight">{builderMode === 'add' ? 'Supplier Intake Form' : 'Update Vendor Master'}</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{builderMode === 'add' ? 'Synchronizing with cloud registry' : `Modifying ${vendor.name}`}</p></div>
                        <button onClick={() => { setViewState('stock'); setActiveFormTab('profile'); }} className="p-3 bg-white text-slate-400 rounded-[2rem] hover:text-slate-600 transition-all border border-slate-200 shadow-sm"><X size={24}/></button>
                    </div>
                            ...
                            <div className="space-y-12 animate-in fade-in duration-200">
                                <section className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                        <div className="sm:col-span-2"><FormRow label="Vendor Name *"><input type="text" className="w-full h-[36px] bg-slate-50 border border-slate-300 rounded-[2rem] px-3 text-xs font-black outline-none focus:ring-4 focus:ring-indigo-500/5 uppercase" placeholder="LEGAL NAME OF SUPPLIER" value={vendor.name || ''} onChange={e => setVendor({...vendor, name: e.target.value.toUpperCase()})} /></FormRow></div>
                                        <div className="sm:col-span-2"><FormRow label="Contact Person / Manager"><input type="text" className="w-full h-[36px] bg-slate-50 border border-slate-300 rounded-[2rem] px-3 text-xs font-bold outline-none" placeholder="FULL NAME" value={vendor.contactPerson || ''} onChange={e => setVendor({...vendor, contactPerson: e.target.value})} /></FormRow></div>
                                        <div className="sm:col-span-4"><FormRow label="Registration / Warehouse Address"><textarea className="w-full min-h-[100px] bg-white border border-slate-300 rounded-[2rem] px-3 py-2 text-sm font-bold outline-none focus:border-indigo-500" placeholder="FULL REGISTERED OFFICE ADDRESS" value={vendor.address || ''} onChange={e => setVendor({...vendor, address: e.target.value})} /></FormRow></div>
                                    </div>
                                </section>
                                ...
                                <section className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                        <FormRow label="GSTIN Number"><input type="text" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black font-mono uppercase" placeholder="GST NUMBER" value={vendor.gstin || ''} onChange={e => setVendor({...vendor, gstin: e.target.value.toUpperCase()})} /></FormRow>
                                        <FormRow label="PAN Number"><input type="text" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black font-mono uppercase" placeholder="PAN NUMBER" value={vendor.panNo || ''} onChange={e => setVendor({...vendor, panNo: e.target.value.toUpperCase()})} /></FormRow>
                                        <FormRow label="DL Number"><input type="text" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black font-mono uppercase" placeholder="DRUG LICENSE" value={vendor.dlNo || ''} onChange={e => setVendor({...vendor, dlNo: e.target.value.toUpperCase()})} /></FormRow>
                                        <FormRow label="UDYAM Number"><input type="text" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black font-mono uppercase" placeholder="UDYAM ID" value={vendor.udyamNo || ''} onChange={e => setVendor({...vendor, udyamNo: e.target.value.toUpperCase()})} /></FormRow>
                                        <FormRow label="CIN Number"><input type="text" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black font-mono uppercase" placeholder="CIN NUMBER" value={vendor.cinNo || ''} onChange={e => setVendor({...vendor, cinNo: e.target.value.toUpperCase()})} /></FormRow>
                                        <FormRow label="Status"><select className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black uppercase" value={vendor.status} onChange={e => setVendor({...vendor, status: e.target.value as any})}><option>Finalized</option><option>Draft</option></select></FormRow>
                                    </div>
                                </section>
                                ...
                                <section className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                        <div className="sm:col-span-2"><FormRow label="Email Address"><input type="email" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-bold" placeholder="VENDOR@SUPPLY.COM" value={vendor.email || ''} onChange={e => setVendor({...vendor, email: e.target.value})} /></FormRow></div>
                                        <div className="sm:col-span-2"><FormRow label="Phone / Mobile"><input type="text" className="w-full h-[36px] bg-white border border-slate-300 rounded-[2rem] px-3 text-xs font-black font-mono" placeholder="+91" value={vendor.phone || ''} onChange={e => setVendor({...vendor, phone: e.target.value})} /></FormRow></div>
                                    </div>
                                </section>
                            </div>
                        )}
                    ...
                    <div className="sticky bottom-0 left-0 right-0 p-6 bg-white/95 backdrop-blur-md border-t border-slate-200 flex justify-end gap-4 shadow-[0_-15px_30px_rgba(0,0,0,0.06)] z-30 shrink-0 px-10">
                        {activeFormTab === 'profile' ? (
                            <>
                                <button onClick={() => { setViewState('stock'); setEditingId(null); }} className="px-10 py-4 bg-slate-100 text-slate-500 rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border border-slate-200 shadow-inner">Cancel</button>
                                <button onClick={handleSave} className="px-16 py-4 bg-gradient-to-r from-indigo-600 to-indigo-400 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-widest shadow-2xl shadow-indigo-500/40 active:scale-95 transition-all hover:brightness-110">Authorize Supplier Entry</button>
                            </>
                        ) : (
                            <button onClick={() => { setViewState('stock'); setEditingId(null); }} className="px-16 py-4 bg-indigo-600 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-widest shadow-xl shadow-indigo-500/20 active:scale-95 transition-all hover:bg-indigo-700">Back to Registry</button>
                        )}
                    </div>
                </div>
            )}
