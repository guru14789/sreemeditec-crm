...
export const VendorModule: React.FC = () => {
    ...
    const handleSaveVendor = async (status: 'Draft' | 'Finalized' = 'Finalized') => {
        ...
        const vendor: Vendor = {
            ...
            cinNo: newVendor.cinNo || '',
            panNo: newVendor.panNo || '',
            dlNo: newVendor.dlNo || '',
            udyamNo: newVendor.udyamNo || '',
            status: status
        };
        await addVendor(vendor);
        setShowAddModal(false);
        setNewVendor({});
        addNotification('Registry Updated', `Vendor "${vendor.name}" added to database as ${status}.`, 'success');
    };
    ...
    const handleUpdateVendor = async () => {
        ...
        try {
            ...
            setSelectedVendor({ ...selectedVendor, ...editVendorData } as Vendor);
            setIsEditing(false);
            addNotification('Registry Updated', `Vendor "${selectedVendor.name}" updated successfully.`, 'success');
        } catch (error) {
                    ...
                    <table className="w-full text-left text-sm text-slate-600">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredVendors.map(vendor => (
                                <tr key={vendor.id} className="hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => { setSelectedVendor(vendor); setEditVendorData(vendor); setIsEditing(false); }}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${vendor.status === 'Draft' ? 'bg-amber-50 border-amber-200 text-amber-600' : 'bg-emerald-50 border-emerald-200 text-emerald-600'}`}>
                            ...
                            {filteredVendors.length === 0 && (
                                <tr><td colSpan={6} className="py-20 text-center text-slate-300 font-bold uppercase tracking-widest opacity-30 italic">No Suppliers Indexed</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden scale-100 animate-in zoom-in-95">
                        ...
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <input type="text" className="w-full border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-medical-500 uppercase" placeholder="CIN NO" value={newVendor.cinNo || ''} onChange={e => setNewVendor({ ...newVendor, cinNo: e.target.value })} />
                                <input type="text" className="w-full border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-medical-500 uppercase" placeholder="PAN NO" value={newVendor.panNo || ''} onChange={e => setNewVendor({ ...newVendor, panNo: e.target.value })} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <input type="text" className="w-full border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-medical-500 uppercase" placeholder="DL no" value={newVendor.dlNo || ''} onChange={e => setNewVendor({ ...newVendor, dlNo: e.target.value })} />
                                <input type="text" className="w-full border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-medical-500 uppercase" placeholder="UDYAM No" value={newVendor.udyamNo || ''} onChange={e => setNewVendor({ ...newVendor, udyamNo: e.target.value })} />
                            </div>
                        ...
                        <div className="p-6 border-t bg-slate-50 flex gap-3">
                            <button onClick={() => handleSaveVendor('Draft')} className="flex-1 bg-amber-600 text-white py-3 rounded-xl font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all text-sm">Save as Draft</button>
                            <button onClick={() => handleSaveVendor('Finalized')} className="flex-[2] bg-medical-600 text-white py-3 rounded-xl font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all text-sm">Save Vendor</button>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col scale-100 animate-in zoom-in-95 overflow-hidden">
                        <div className="p-6 border-b flex justify-between items-center bg-slate-50 rounded-t-3xl shrink-0">
                            ...
                            <div className="flex items-center gap-4">
                                {!isEditing && (
                                    <button onClick={() => setIsEditing(true)} className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition-all">Edit Supplier</button>
                                )}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                        <div className="space-y-1">
                                            ...
                                            <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" value={editVendorData.name || ''} onChange={e => setEditVendorData({ ...editVendorData, name: e.target.value })} />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Contact Person</label>
                                            <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" value={editVendorData.contactPerson || ''} onChange={e => setEditVendorData({ ...editVendorData, contactPerson: e.target.value })} />
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Phone</label>
                                                <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" value={editVendorData.phone || ''} onChange={e => setEditVendorData({ ...editVendorData, phone: e.target.value })} />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email</label>
                                                <input type="email" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" value={editVendorData.email || ''} onChange={e => setEditVendorData({ ...editVendorData, email: e.target.value })} />
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">CIN NO</label>
                                                <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500 uppercase" value={editVendorData.cinNo || ''} onChange={e => setEditVendorData({ ...editVendorData, cinNo: e.target.value })} />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">PAN NO</label>
                                                <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500 uppercase" value={editVendorData.panNo || ''} onChange={e => setEditVendorData({ ...editVendorData, panNo: e.target.value })} />
                                            </div>
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                ...
                                                <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500 uppercase" value={editVendorData.dlNo || ''} onChange={e => setEditVendorData({ ...editVendorData, dlNo: e.target.value })} />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">UDYAM No</label>
                                                <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500 uppercase" value={editVendorData.udyamNo || ''} onChange={e => setEditVendorData({ ...editVendorData, udyamNo: e.target.value })} />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">GSTIN</label>
                                            <input type="text" className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" value={editVendorData.gstin || ''} onChange={e => setEditVendorData({ ...editVendorData, gstin: e.target.value })} />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Address</label>
                                            <textarea className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" rows={3} value={editVendorData.address || ''} onChange={e => setEditVendorData({ ...editVendorData, address: e.target.value })} />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Status</label>
                                            <select className="w-full border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500" value={editVendorData.status || 'Finalized'} onChange={e => setEditVendorData({ ...editVendorData, status: e.target.value as any })}>
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-4">
                                        ...
                                        <div className="space-y-4 text-sm text-slate-600 font-medium">
                                            ...
                                            <div className="pt-2 mt-2 border-t border-slate-200 space-y-2">
                                                ...
                                                <div className="flex items-center justify-between text-[10px] uppercase tracking-widest"><span className="text-slate-400">UDYAM:</span> <span className="font-bold text-slate-800">{selectedVendor.udyamNo || 'N/A'}</span></div>
                                            </div>
                                        </div>
                                    </div>
