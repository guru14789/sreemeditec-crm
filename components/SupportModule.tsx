...
export const SupportModule: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col gap-4 overflow-hidden p-2">
            <div className="bg-slate-100 p-1.5 rounded-[2.5rem] border border-slate-200 shadow-inner w-fit shrink-0 flex gap-1">
                <button onClick={() => setViewState('stock')} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-[2rem] transition-all flex items-center gap-2 ${viewState === 'stock'  ? 'bg-emerald-900 text-white shadow-[0_10px_20px_-5px_rgba(6,78,59,0.5)] scale-100' : 'text-slate-400 hover:text-emerald-700 scale-95'}`}><MessageSquare size={16} /> Console</button>
                <button onClick={() => setViewState('builder')} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-[2rem] transition-all flex items-center gap-2 ${viewState === 'builder'  ? 'bg-emerald-900 text-white shadow-[0_10px_20px_-5px_rgba(6,78,59,0.5)] scale-100' : 'text-slate-400 hover:text-emerald-700 scale-95'}`}><Plus size={16} /> New Case</button>
            </div>
                    ...
                    <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-300 shadow-sm overflow-hidden flex flex-col animate-in fade-in">
                        <div className="p-5 border-b border-slate-300 bg-slate-50/30 flex flex-col sm:flex-row justify-between items-center shrink-0 gap-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-blue-100 rounded-[2rem] flex items-center justify-center text-blue-600 shadow-inner border border-blue-50"><Headset size={20} /></div>
                                ...
                                {filteredTickets.map(ticket => (
                                    <div key={ticket.id} onClick={() => setSelectedTicket(ticket)} className={`p-6 cursor-pointer transition-all hover:bg-slate-50 flex items-center gap-6 group ${selectedTicket?.id === ticket.id ? 'bg-medical-50/50 border-l-4 border-medical-500' : 'border-l-4 border-transparent'}`}>
                                        ...
                                        <div className="shrink-0 flex items-center gap-4">
                                            <div className={`px-4 py-2 rounded-[2rem] border text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${ticket.status === 'Open' ? 'bg-blue-50 text-blue-600 border-blue-100 shadow-sm' : ticket.status === 'In Progress' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
 ...
 <h3 className="text-xl font-bold tracking-tight text-slate-800 uppercase tracking-tight leading-tight mb-2">{selectedTicket.subject}</h3>
                            ...
                            <div className="p-6 bg-white border-t border-slate-200">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <button onClick={handleAiSuggest} disabled={isGenerating} className="px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-[2rem] border border-indigo-100 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-100 transition-all disabled:opacity-50">{isGenerating ? <RefreshCw size={12} className="animate-spin" /> : <Sparkles size={12} />} AI Synth</button>
                                </div>
                                <div className="relative group">
                                    ...
                                    <button onClick={handleSendReply} disabled={!replyText.trim()} className="absolute bottom-4 right-4 p-3 bg-medical-600 text-white rounded-[2rem] shadow-xl shadow-medical-500/20 hover:brightness-110 active:scale-95 transition-all disabled:opacity-50 disabled:grayscale"><Send size={20}/></button>
                                </div>
                        ...
                        <div className="hidden lg:flex flex-1 flex-col items-center justify-center bg-white rounded-[2.5rem] border border-dashed border-slate-300 animate-in fade-in">
                            ...
                            <h3 className="text-lg font-playfair font-bold tracking-tight text-slate-300 uppercase tracking-[0.4em]">Station Awaiting</h3>
                ...
                <div className="flex-1 flex flex-col bg-white rounded-[2.5rem] shadow-2xl border border-slate-300 overflow-hidden animate-in slide-in-from-bottom-6">
                    <div className="flex bg-slate-50/80 backdrop-blur-sm border-b border-slate-300 shrink-0 px-10 py-6 justify-between items-center">
                        <div className="flex flex-col"><h3 className="text-2xl font-playfair font-bold tracking-tight text-slate-800 uppercase tracking-tight">Manual Case Creation</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Initializing offline support ticket node</p></div>
                    ...
                    <div className="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar pb-32">
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="sm:col-span-4"><FormRow label="Primary Case Documentation"><textarea className="w-full min-h-[120px] bg-white border border-slate-300 rounded-[2rem] px-3 py-2 text-sm font-bold outline-none focus:border-medical-500 uppercase" placeholder="ENTER DETAILED LOG OF THE ISSUE AS REPORTED..." /></FormRow></div>
                        </section>
                    </div>
                    ...
                    <div className="sticky bottom-0 left-0 right-0 p-6 bg-white/95 backdrop-blur-md border-t border-slate-200 flex justify-end gap-4 shadow-[0_-15px_30px_rgba(0,0,0,0.06)] z-30 shrink-0 px-10">
                        <button onClick={() => setViewState('stock')} className="px-10 py-4 bg-slate-100 text-slate-500 rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border border-slate-200 shadow-inner">Abort Creation</button>
                        <button onClick={handleCreateTicket} className="px-16 py-4 bg-gradient-to-r from-medical-600 to-blue-500 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-widest shadow-2xl shadow-medical-500/40 active:scale-95 transition-all hover:brightness-110">Authorize Case Ticket</button>
                    </div>
                </div>
            )}
        </div>
