...
export const ServiceOrderModule: React.FC = () => {
    ...
    const [order, setOrder] = useState<Partial<Invoice>>({
        ...
        cpoNumber: '', // Repurposed for SMPSR NO
        cpoDate: new Date().toISOString().split('T')[0], // Repurposed for SMPSR DATE
        items: [],
        discount: 0,
        status: 'Pending',
        customerName: '',
    ...
    const renderSOTemplate = (data: Partial<Invoice>, totals: { subTotal: number, taxTotal: number, totalWithGst: number, discount: number, grandTotal: number }) => (
        <div className="bg-white p-[10mm] text-black w-full min-h-full flex flex-col border border-slate-300 shadow-sm" style={{ fontFamily: 'Calibri, "Segoe UI", Candara, Segoe, Optima, Arial, sans-serif' }}>
            ...
            <div className="border border-black text-center py-1 mb-2 bg-slate-50">
                <h2 className="text-sm font-bold uppercase tracking-widest">CUSTOMER SERVICE ORDER</h2>
            </div>
            ...
            <div className="grid grid-cols-2 border-x border-t border-black text-[12px] min-h-[50px]">
                <div className="border-r border-black p-1.5"><p className="font-bold mb-1 uppercase underline decoration-slate-300">Customer and Address:</p><p className="font-bold uppercase">{data.customerName}</p><p className="whitespace-pre-wrap">{data.customerAddress}</p></div>
                <div className="p-1.5"><p className="font-bold mb-1 uppercase underline decoration-slate-300">Delivery Address:</p><p className="whitespace-pre-wrap leading-tight">{data.deliveryAddress || 'Same as billing.'}</p></div>
            </div>
            ...
            <div className="border border-black">
                <div className="grid grid-cols-[1fr_24.1mm] border-b border-black text-[12px]"><div className="p-1.5 text-left border-r border-black font-bold uppercase">Total Spares</div><div className="p-1.5 text-left font-black">{totals.totalWithGst.toLocaleString()}</div></div>
                <div className="grid grid-cols-[1fr_24.1mm] border-b border-black text-[12px]"><div className="p-1.5 text-left border-r border-black font-bold uppercase">Discount/buyback/adjustment:</div><div className="p-1.5 text-left font-bold">{totals.discount.toLocaleString()}</div></div>
                <div className="grid grid-cols-[1fr_24.1mm] border-b border-black text-[12px]"><div className="p-1.5 text-left border-r border-black font-bold text-sm uppercase">Grand Total (Incl Service Charges)</div><div className="p-1.5 text-left font-black text-sm">{totals.grandTotal.toLocaleString()}</div></div>
                <div className="p-1.5 text-[12px] italic border-b border-black last:border-b-0"><span className="font-bold uppercase">Advance payment details:</span> <span className="ml-2">{data.advanceAmount ? `Rs. ${data.advanceAmount.toLocaleString()} received via ${data.paymentMethod} on ${formatDateDDMMYYYY(data.advanceDate)}` : 'Not recorded.'}</span></div>
            </div>
            ...
            <div className="grid grid-cols-2 border-x border-b border-black text-[12px] flex-1"><div className="border-r border-black p-2 flex flex-col justify-between min-h-[100px]"><p className="font-bold">Customer seal and signature:</p><div className="flex-1 flex items-end pb-2"><div className="w-full border-t border-dotted border-black pt-1 text-[10px] uppercase font-black text-slate-400">Authorised Signatory & Seal</div></div></div><div className="p-2 flex flex-col justify-between relative overflow-hidden min-h-[100px]"><div className="relative z-10"><p className="font-bold text-left uppercase">For Sree Meditec Representative:</p></div><div className="flex justify-between items-end relative z-10 pt-10"><div className="text-left"><div className="h-12 w-full"></div></div><div className="w-24 h-24 border-4 border-slate-100 rounded-full flex items-center justify-center text-[8px] font-black text-slate-200 uppercase text-center rotate-12 select-none -mr-4 -mb-4 opacity-40">Official<br/>Company Seal</div></div></div></div>
        </div>
    );
            ...
            {viewState === 'history' ? (
                ...
                <div className="flex-1 flex flex-col bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden animate-in slide-in-from-bottom-4">
                    ...
                    <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className={`w-full lg:w-1/2 bg-slate-100 border-l border-slate-200 flex flex-col lg:overflow-hidden ${builderTab === 'form' ? 'hidden lg:flex' : 'flex'}`}><div className="hidden lg:flex bg-[#81D7D3] p-1 shrink-0"><button onClick={() => setBuilderTab('preview')} className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all ${builderTab === 'preview' ? 'bg-white text-teal-700 shadow-sm' : 'text-white hover:bg-white/10'}`}>Live Order Preview</button><button onClick={() => setBuilderTab('catalog')} className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all ${builderTab === 'catalog' ? 'bg-white text-teal-700 shadow-sm' : 'text-white hover:bg-white/10'}`}>Parts Catalog</button></div><div className="flex-1 overflow-hidden relative"><div className={`h-full overflow-y-auto p-4 md:p-8 flex justify-center custom-scrollbar ${builderTab === 'preview' ? 'flex' : 'hidden'}`}><div className="shadow-2xl h-fit w-full max-w-[210mm] transition-all duration-300">{renderSOTemplate(order, totals)}</div></div><div className={`h-full bg-white flex flex-col p-6 overflow-hidden animate-in fade-in ${builderTab === 'catalog' ? 'flex' : 'hidden'}`}><div className="flex justify-between items-center mb-6"><h3 className="font-black text-slate-800 uppercase tracking-tight">Select Spares</h3><div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Filter Spares..." className="pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none w-48" value={catalogSearch} onChange={e => setCatalogSearch(e.target.value)} /></div></div><div className="flex-1 overflow-y-auto custom-scrollbar grid grid-cols-1 gap-4">{filteredProducts.map(prod => (<div key={prod.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl hover:border-teal-400 transition-all cursor-pointer flex justify-between items-center group" onClick={() => handleAddItem(prod)}><div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className="text-[8px] font-black uppercase text-indigo-600 bg-white px-1.5 py-0.5 rounded border border-indigo-100">{prod.category}</span><span className="text-[10px] font-mono text-slate-400">{prod.sku}</span></div><h4 className="font-black text-slate-800 text-sm group-hover:text-teal-700 transition-colors">{prod.name}</h4><p className="text-[10px] text-slate-400 font-bold uppercase mt-1">Available: {prod.stock} units</p></div><div className="ml-4 p-1.5 bg-white rounded-lg border border-slate-100 group-hover:bg-teal-600 group-hover:text-white transition-all shadow-sm"><Plus size={16} /></div></div>))}</div></div></div></div>
                    </div>
                </div>
            )}
