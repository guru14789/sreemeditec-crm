    ...
    <div className="h-full flex flex-col gap-6 overflow-y-auto lg:overflow-hidden p-2">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
        <div className="bg-white p-5 rounded-3xl border border-red-100 shadow-sm flex items-center gap-4">
            <div className="bg-red-50 p-3 rounded-[2rem] text-red-600 shadow-sm"><AlertTriangle size={24}/></div>
            <div><div className="text-3xl font-playfair font-bold tracking-tight text-slate-800">{serviceTickets.filter(t => t.priority === 'High').length}</div><div className="text-[10px] font-black text-red-400 uppercase tracking-widest">Active Breakdowns</div></div>
        </div>
        <div className="bg-white p-5 rounded-3xl border border-medical-100 shadow-sm flex items-center gap-4">
            <div className="bg-medical-50 p-3 rounded-[2rem] text-medical-600 shadow-sm"><Wrench size={24}/></div>
            <div><div className="text-3xl font-playfair font-bold tracking-tight text-slate-800">{serviceTickets.filter(t => t.status !== 'Resolved').length}</div><div className="text-[10px] font-black text-medical-400 uppercase tracking-widest">Tickets In Queue</div></div>
        </div>
        <div className="bg-white p-5 rounded-3xl border border-emerald-100 shadow-sm flex items-center gap-4">
            <div className="bg-emerald-50 p-3 rounded-[2rem] text-emerald-600 shadow-sm"><Calendar size={24}/></div>
 <div><div className="text-3xl font-bold tracking-tight text-slate-800">12</div><div className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">AMC Renewals (Oct)</div></div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-3xl shadow-sm border border-slate-300 flex flex-col overflow-hidden min-h-[500px]">
        <div className="p-4 border-b border-slate-300 flex justify-between items-center bg-slate-50/50">
            <div className="flex bg-slate-100 p-1.5 rounded-[2rem]">
                <button onClick={() => setActiveTab('tickets')} className={`px-6 py-2 rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'tickets' ? 'bg-white text-medical-700 shadow-sm' : 'text-slate-400'}`}>Tickets</button>
                <button onClick={() => setActiveTab('amc')} className={`px-6 py-2 rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'amc' ? 'bg-white text-medical-700 shadow-sm' : 'text-slate-400'}`}>Renewals</button>
                <button onClick={() => setActiveTab('reports')} className={`px-6 py-2 rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'reports' ? 'bg-white text-medical-700 shadow-sm' : 'text-slate-400'}`}>Reports</button>
            </div>
            {activeTab === 'reports' && (
                 <button onClick={() => setShowReportModal(true)} className="bg-medical-600 text-white px-4 py-2 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
            ...
            {activeTab === 'tickets' && (
                <div className="space-y-4">
                    {serviceTickets.map(ticket => (
                        <div key={ticket.id} className="bg-white p-5 rounded-[2rem] border border-slate-300 shadow-sm hover:shadow-lg transition-all flex flex-col md:flex-row gap-4 group">
                            ...
                            <div className="flex flex-col justify-between items-end border-l border-slate-50 pl-4">
                                <span className="bg-slate-50 text-slate-500 px-3 py-1 rounded-[2rem] text-[10px] font-black uppercase border border-slate-300">{ticket.status}</span>
                            </div>
                        </div>
                    ))}
            ...
            {activeTab === 'reports' && (
                <div className="grid grid-cols-1 gap-4">
                    {serviceReports.filter(r => r.documentType === 'ServiceReport').map(report => (
                         <div key={report.id} className="bg-white p-5 rounded-[2rem] border border-slate-300 shadow-sm flex items-center gap-4">
                            <div className="bg-slate-50 p-4 rounded-[2rem] text-medical-600"><FileText size={24}/></div>
                            <div className="flex-1">
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-in fade-in">
              <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden scale-100 animate-in zoom-in-95">
                  <div className="p-6 border-b border-slate-300 flex justify-between items-center bg-slate-50/50 rounded-t-[2rem]">
                      <h3 className="text-xl font-playfair font-bold tracking-tight text-slate-800 uppercase tracking-tight">Technical Visit Log</h3>
                  ...
                  <div className="p-6 border-t border-slate-300 flex gap-3">
                      <button onClick={() => setShowReportModal(false)} className="flex-1 bg-slate-100 py-3 rounded-[2rem] text-[10px] font-black uppercase">Cancel</button>
                      <button onClick={handleSaveReport} className="flex-1 bg-medical-600 text-white py-3 rounded-[2rem] text-[10px] font-black uppercase shadow-lg">Save Report</button>
                  </div>
              </div>
          </div>
      )}
