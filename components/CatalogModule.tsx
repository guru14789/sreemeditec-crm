...
import { Product, BrandDetail, ModelDetail } from '../types';
...
export const CatalogModule: React.FC = () => {
    ...
    // Helper to calculate price ranges, brands, and total stock for a master product card
    const productStats = useMemo(() => {
        ...
        products.forEach(p => {
            let minPrice = p.sellingPrice || 0;
            let maxPrice = p.sellingPrice || 0;
            ...
            if (p.brands && p.brands.length > 0) {
                ...
                p.brands.forEach(b => {
                    brands.push(b.name);
                    b.models?.forEach(m => {
                        const mStock = m.vendors?.reduce((sum, v) => sum + (Number(v.stock) || 0), 0) || 0;
                        totalStock += mStock;
                        
                        const primaryVendor = m.vendors && m.vendors.length > 0 ? m.vendors[0] : null;
                        const sPrice = primaryVendor ? primaryVendor.sellingPrice : (p.sellingPrice || 0);
                        prices.push(sPrice);
                    });
                });
                if (prices.length > 0) {
    ...
    const filteredProducts = useMemo(() => {
        return products.filter(product => {
            ...
            const brandMatches = stats?.brandsList.some(b => b.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    }, [products, searchQuery, selectedCategory, productStats]);

    const calculateGstPrice = (price: number, taxRate: number = 18) => {
        return price + (price * (taxRate / 100));
    };

    // Open detailed modal and initialize defaults
    const handleOpenModal = (product: Product) => {
        setSelectedProduct(product);
        if (product.brands && product.brands.length > 0) {
            const firstBrand = product.brands[0];
            setActiveBrandName(firstBrand.name);
            if (firstBrand.models && firstBrand.models.length > 0) {
                setActiveModelName(firstBrand.models[0].name);
            } else {
                setActiveModelName('');
            }
        } else {
    ...
    // Active selected Brand and Model details in the modal
    const modalDetails = useMemo(() => {
        if (!selectedProduct) return null;
        
        const matchedBrand = selectedProduct.brands?.find(b => b.name === activeBrandName);
        const matchedModel = matchedBrand?.models?.find(m => m.name === activeModelName);

        // Derive price and stock
        const primaryVendor = matchedModel?.vendors && matchedModel.vendors.length > 0 ? matchedModel.vendors[0] : null;
        const sellingPrice = primaryVendor ? primaryVendor.sellingPrice : (selectedProduct.sellingPrice || 0);
        const taxRate = primaryVendor ? primaryVendor.gstRate : (selectedProduct.taxRate || 18);
        const sku = primaryVendor ? primaryVendor.sku : (selectedProduct.sku || '');
        const stock = matchedModel?.vendors?.reduce((sum, v) => sum + (Number(v.stock) || 0), 0) ?? (selectedProduct.stock || 0);
        ...
        return {
            ...
            sku,
            stock,
            specs: matchedModel?.specs || []
        };
    }, [selectedProduct, activeBrandName, activeModelName]);
    ...
    return (
        <div className="h-full flex flex-col gap-6 overflow-hidden">
                ...
                {viewMode === 'grid' ? (
                                ...
                                >
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-start gap-2">
                                            <div className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-[2rem] text-[9px] font-black text-slate-500 uppercase tracking-widest truncate">{product.category}</div>
                                            <div className={`px-2.5 py-0.5 rounded-[2rem] text-[9px] font-black uppercase tracking-widest whitespace-nowrap ${stats.totalStock > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                                {stats.totalStock > 0 ? `${stats.totalStock} IN STOCK` : 'OUT OF STOCK'}
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            {/* Brands List */}
                                            {stats.brandsList.length > 0 && (
                                                <div className="flex flex-wrap gap-1.5 mt-2">
                                                    {stats.brandsList.map((bName, idx) => (
                                                        <span key={idx} className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-[2rem] text-[8px] font-black text-slate-500 uppercase tracking-wider">
                                                            {bName}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        <div className="flex justify-between items-end">
                                            ...
                                            <div className="text-right">
                                                <p className="text-[9px] md:text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1 md:mb-1.5 flex items-center justify-end gap-1.5">
                                                    Inclusive Range <Tag size={12} />
                                                </p>
                                                <p className="text-sm md:text-base font-bold tracking-tight text-slate-900 dark:text-white tracking-tighter">
                                                    ₹{calculateGstPrice(stats.minPrice, product.taxRate).toLocaleString('en-IN', { maximumFractionDigits: 0 })} - ₹{calculateGstPrice(stats.maxPrice, product.taxRate).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                        </div>
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="hidden md:flex bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-[2rem] p-4 px-8 items-center text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">
                            ...
                            <div className="flex-1 text-center">Price Range</div>
                            <div className="flex-1 text-center">Tax</div>
                            <div className="flex-1 text-right">Total Stock</div>
                        </div>
                                ...
                                >
                                            ...
                                            <div className="flex items-center gap-2 mt-1.5 overflow-hidden">
                                                ...
                                                {stats.brandsList.length > 0 && (
                                                    <>
                                                        ...
                                                        <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest truncate">Brands: {stats.brandsList.join(', ')}</span>
                                                    </>
                                                )}
                                                <span className="w-1 h-1 rounded-full bg-slate-300 shrink-0"></span>
                                                <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest truncate">{product.sku}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-3 items-center justify-between md:contents">
                                        <div className="md:flex-1 md:text-center">
                                            <span className="md:hidden text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Price Range</span>
                                        ...
                                        <div className="md:flex-1 text-right">
                                            <span className="md:hidden text-[8px] font-black text-emerald-600 uppercase tracking-widest block mb-1">Total Stock</span>
                                            <div className="inline-flex flex-col items-end">
                                                <span className={`text-xs font-black uppercase ${stats.totalStock > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                    {stats.totalStock} {product.unit || 'nos'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
            ...
            {/* Detailed Brand/Model Selection Modal */}
            {selectedProduct && (
                <div 
                ...
                >
                    <div 
                        className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 animate-out zoom-out-95"
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                            {/* Brand and Model selectors inside modal if product has brands */}
                            {selectedProduct.brands && selectedProduct.brands.length > 0 && (
                                <div className="space-y-4 bg-slate-50 dark:bg-slate-850 p-4 rounded-3xl border border-slate-150 dark:border-slate-800">
                                    {/* 1. Choose Brand */}
                                    <div>
                                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider block mb-2">1. Choose Brand</label>
                                        <div className="flex flex-wrap gap-2">
                                            {selectedProduct.brands.map(b => (
                                                <button
                                                    key={b.id}
                                                    onClick={() => {
                                                        setActiveBrandName(b.name);
                                                        if (b.models && b.models.length > 0) {
                                                            setActiveModelName(b.models[0].name);
                                                        } else {
                                                            setActiveModelName('');
                                                        }
                                                    }}
                                                    className={`px-4 py-2 text-xs font-black rounded-xl border transition-all ${
                                                        activeBrandName === b.name
                                    ...
                                    {/* 2. Choose Model */}
                                    {(() => {
                                        const currentBrand = selectedProduct.brands.find(b => b.name === activeBrandName);
                                        if (!currentBrand || !currentBrand.models || currentBrand.models.length === 0) return null;
                                        return (
                                            <div>
                                                <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider block mb-2">2. Choose Model</label>
                                                <div className="flex flex-wrap gap-2">
                            ...
                            {/* Spec Details of selected Brand & Model */}
                            {modalDetails && (
                                <div className="space-y-4">
                                        ...
                                        <div className="p-5 bg-slate-50 dark:bg-slate-800/40 rounded-3xl border border-slate-100 dark:border-slate-800/80 space-y-4">
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex justify-between items-center py-1 border-b border-slate-100 dark:border-slate-800/60">
                                                    ...
                                                    <span className={`text-xs font-black uppercase ${modalDetails.stock > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                        {modalDetails.stock} {selectedProduct.unit || 'units'}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between items-center py-1 border-b border-slate-100 dark:border-slate-800/60">
                                                    <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</span>
                                                    <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest ${modalDetails.stock > 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-950/50 dark:text-rose-400'}`}>
                                                        {modalDetails.stock > 0 ? 'In Stock' : 'Out of Stock'}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between items-center py-1 border-b border-slate-100 dark:border-slate-800/60">
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex justify-between items-center py-1 border-b border-slate-100 dark:border-slate-800/60">
                                                    ...
                                                    <span className="text-xs font-black text-slate-700 dark:text-slate-200">
                                                        ₹{modalDetails.sellingPrice.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between items-center py-1 border-b border-slate-100 dark:border-slate-800/60">
                                                    ...
                                                    <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                                                        <Percent size={10} /> {modalDetails.taxRate}%
                                                    </span>
                                                </div>
                                                <div className="flex justify-between items-center py-1">
                                                    ...
                                                    <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">
                                                        ₹{calculateGstPrice(modalDetails.sellingPrice, modalDetails.taxRate).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {modalDetails.specs.length > 0 ? (
                                            <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                                                ...
                                                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                                    {modalDetails.specs.map((spec, idx) => (
                                            ...
                                            <p className="text-xs font-medium text-slate-550 leading-relaxed">
                                                No detailed technical specifications or description available for this model.
                                            </p>
                                        )}
                                    </div>
                                </div>
