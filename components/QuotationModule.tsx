...
export const QuotationModule: React.FC = () => {
    ...
    const handleDownloadPDF = (data: Partial<Invoice>) => {
        ...
        autoTable(doc, {
            ...
            columnStyles: { 
                0: { cellWidth: 35 }, 1: { cellWidth: 20 }, 2: { cellWidth: 35 },
                3: { cellWidth: 10, halign: 'center' }, 4: { cellWidth: 18, halign: 'right' }, 
        ...
        if (signature) {
            doc.addImage(signature, 'PNG', 15, signOffY + 16, 35, 12);
        }
        if (seal) {
            ...
            {viewState === 'history' ? (
                <div className="flex-1 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col animate-in fade-in">
                    <div className="p-4 border-b border-slate-100 bg-slate-50/30 flex justify-between items-center">
                        <h3 className="font-black text-slate-800 uppercase tracking-widest text-xs">Quotations Registry</h3>
                    </div>
                    <div className="flex-1 overflow-auto custom-scrollbar">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 sticky top-0 z-10 font-bold uppercase text-[10px] text-slate-500 border-b">
                                <tr><th className="px-6 py-4">Reference</th><th className="px-6 py-4">Consignee</th><th className="px-6 py-4 text-right">Grand Total</th><th className="px-6 py-4 text-center">Status</th><th className="px-6 py-4 text-right">Action</th></tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">{invoices.filter(i => i.documentType === 'Quotation').map(inv => (<tr key={inv.id} className="hover:bg-slate-50 transition-colors group"><td className="px-6 py-4 font-black">{inv.invoiceNumber}</td><td className="px-6 py-4 font-bold text-slate-700 uppercase">{inv.customerName}</td><td className="px-6 py-4 text-right font-black text-teal-700">₹{inv.grandTotal.toLocaleString()}</td><td className="px-6 py-4 text-center"><span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${inv.status === 'Draft' ? 'bg-slate-100 text-slate-500' : 'bg-emerald-50 text-emerald-700'}`}>{inv.status}</span></td><td className="px-6 py-4 text-right"><div className="flex justify-end gap-2"><button onClick={() => { setQuote(inv); setEditingId(inv.id); setViewState('builder'); }} className="p-2 text-slate-400 hover:text-indigo-600"><Edit size={18}/></button><button onClick={() => handleDownloadPDF(inv)} className="p-2 text-slate-400 hover:text-emerald-500"><Download size={18}/></button></div></td></tr>))}</tbody>
                        </table>
                    </div>
                </div>
            ) : (
                        ...
                        {builderTab === 'form' && (
                            <div className="h-full overflow-y-auto p-4 md:p-8 space-y-12 custom-scrollbar bg-white">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-4">
                                            <div className="space-y-1.5">
                                                ...
                                                <input type="text" list="client-list" className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus:ring-4 focus:ring-medical-500/5" value={quote.customerName || ''} onChange={e => handleClientSelect(e.target.value)} placeholder="Search or Enter Client Name" />
                                            </div>
                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Client GST Number</label>
                                                <input type="text" className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none" value={quote.customerGstin} onChange={e => setQuote({...quote, customerGstin: e.target.value})} placeholder="GSTIN" />
                                            </div>
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Client Address</label>
                                            <textarea rows={4} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none resize-none" value={quote.customerAddress || ''} onChange={e => setQuote({...quote, customerAddress: e.target.value})} placeholder="Full site or billing address..." />
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section className="space-y-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Subject Line</label>
                                        <input type="text" className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold outline-none" value={quote.subject} onChange={e => setQuote({...quote, subject: e.target.value})} placeholder="e.g. Ultrasound Gel (5L)" />
                                    </div>
                                </section>
                                            ...
                                            <div key={item.id} className="p-4 sm:p-5 bg-slate-50 border border-slate-200 rounded-[1.5rem] sm:rounded-[2rem] relative group hover:bg-white hover:border-medical-200 transition-all">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                                                    <div className="md:col-span-6 space-y-1">
                                                        ...
                                                        <input type="text" list="prod-list" className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold" value={item.description} onChange={e => updateItem(item.id, 'description', e.target.value)} />
                                                    </div>
                                                    <div className="md:col-span-6 space-y-1">
                                ...
                                <section className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Freight (₹)</label>
                                ...
                                <section className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Payment Terms</label>
                                            <textarea rows={3} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold outline-none resize-none" value={quote.paymentTerms} onChange={e => setQuote({...quote, paymentTerms: e.target.value})} />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Delivery Terms</label>
                                            <textarea rows={3} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold outline-none resize-none" value={quote.deliveryTerms} onChange={e => setQuote({...quote, deliveryTerms: e.target.value})} />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Warranty Terms</label>
                                            <textarea rows={3} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold outline-none resize-none" value={quote.warrantyTerms} onChange={e => setQuote({...quote, warrantyTerms: e.target.value})} />
                                        </div>
                                    </div>
                                </section>
                                ...
                                <div className="shadow-2xl h-fit transition-all duration-300 origin-top scale-[0.5] sm:scale-[0.7] md:scale-[0.8] lg:scale-[0.7] xl:scale-[0.85] 2xl:scale-[0.95]" style={{ width: '210mm' }}>
                                    <div className="bg-white p-[15mm] text-black w-full min-h-[297mm] flex flex-col border border-slate-300 shadow-2xl mx-auto" style={{ fontFamily: 'Calibri, sans-serif' }}>
                                        ...
                                        <div className="text-center mb-4">
                                            ...
                                            <h1 className="text-4xl font-bold uppercase mb-1">SREE MEDITEC</h1>
                                            <p className="text-[10px] font-semibold">New No: 18, Old No: 2, Bajanai Koil Street, Rajajipakkam, Chennai 600 073.</p>
                                            <p className="text-[10px] font-semibold">Mob: 9884818398.</p>
                                            <p className="text-[10px] font-bold mt-1">GST NO: 33APGPS4675G2ZL</p>
                                        </div>

                                        <div className="text-center mb-8"><h2 className="text-xl font-bold underline uppercase tracking-widest">Quotation</h2></div>
                                                ...
                                                {(quote.items || []).map(it => {
                                                    ...
                                                    return (
                                                        <tr key={it.id} className="border-b border-black text-center align-top">
                                                            ...
                                                            <td className="border-r border-black p-1 text-right">Rs.{it.unitPrice.toFixed(2)}</td>
                                                            <td className="border-r border-black p-1">{it.taxRate}%</td>
                                                            <td className="border-r border-black p-1 text-right">Rs.{rowGst.toFixed(2)}</td>
                                                            <td className="p-1 text-right font-bold">
                                                                <div>Rs.{rowTotal.toFixed(2)}</div>
                                        ...
                                        <div className="flex flex-col items-end text-xs font-bold mb-10 space-y-1">
                                            <div className="w-[200px] flex justify-between border-b border-slate-200 py-1"><span>Gross Total:</span><span>Rs.{totals.subtotal.toFixed(2)}</span></div>
                                            <div className="w-[200px] flex justify-between border-b border-slate-200 py-1"><span>Sub Total (Taxable):</span><span>Rs.{(totals.subtotal - (quote.discount || 0)).toFixed(2)}</span></div>
                                            <div className="w-[200px] flex justify-between border-b border-slate-200 py-1"><span>Total GST:</span><span>Rs.{(totals.itemGstTotal + totals.freightGst).toFixed(2)}</span></div>
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                                    ...
                                    {filteredCatalog.length === 0 && (
                                        <div className="col-span-full py-12 flex flex-col items-center justify-center text-slate-400">
                                            ...
                                            <p className="text-xs font-black uppercase">No matching products</p>
                                        </div>
                                    )}
                                </div>
                            </div>
